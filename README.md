...
[![CI](https://github.com/pi-os/internet-pi/workflows/CI/badge.svg?event=push)](https://github.com/pi-os/internet-pi/actions?query=workflow%3ACI)
...
Other features:
  ...
  - **Shelly Plug Monitoring**: Installs a [`shelly-plug-prometheus` exporter](https://github.com/pi-os/shelly-plug-prometheus) and a Grafana dashboard, which tracks and displays power usage on a Shelly Plug running on the local network. (Disabled by default. Enable and configure using the `shelly_plug_*` vars in `config.yml`.)
  - **AirGradient Monitoring**: Configures [`airgradient-prometheus`](https://github.com/pi-os/airgradient-prometheus) and a Grafana dashboard, which tracks and displays air quality over time via one or more AirGradient DIY monitors. (Disabled by default. Enable and configure using the `airgradient_enable` var in `config.yml`. See example configuration for ability to monitor multiple AirGradient DIY stations.)
...
## Setup
  ...
  2. Clone this repository: `git clone https://github.com/pi-os/internet-pi.git`, then enter the repository directory: `cd internet-pi`.
...
A guide for backing up the configurations and historical data will be posted here as part of [Issue #194: Create Backup guide](https://github.com/pi-os/internet-pi/issues/194).
